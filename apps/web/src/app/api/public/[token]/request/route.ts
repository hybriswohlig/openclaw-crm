import { NextRequest, NextResponse } from "next/server";
import {
  recordCustomerRequest,
  type CustomerRequestKind,
} from "@/services/customer-portal-data";

export const dynamic = "force-dynamic";

function clientIp(req: NextRequest): string {
  const xff = req.headers.get("x-forwarded-for");
  if (xff) return xff.split(",")[0].trim();
  return req.headers.get("x-real-ip") ?? "0.0.0.0";
}

export async function POST(
  req: NextRequest,
  { params }: { params: Promise<{ token: string }> }
) {
  const { token } = await params;
  let body: { kind?: CustomerRequestKind; message?: string; preferredDates?: string[] };
  try {
    body = (await req.json()) as typeof body;
  } catch {
    return NextResponse.json({ error: { code: "INVALID_INPUT" } }, { status: 400 });
  }

  const kind = body.kind;
  if (kind !== "reschedule" && kind !== "question" && kind !== "damage") {
    return NextResponse.json({ error: { code: "INVALID_INPUT" } }, { status: 400 });
  }

  const result = await recordCustomerRequest(
    token,
    {
      kind,
      message: typeof body.message === "string" ? body.message : "",
      preferredDates: body.preferredDates,
    },
    { ipAddress: clientIp(req), userAgent: req.headers.get("user-agent") ?? "" }
  );

  if (!result.ok) {
    const status =
      result.reason === "not_found"
        ? 404
        : result.reason === "revoked"
          ? 410
          : result.reason === "rate_limited"
            ? 429
            : 400;
    return NextResponse.json({ error: { code: result.reason.toUpperCase() } }, { status });
  }
  return NextResponse.json({ data: { ok: true } });
}
